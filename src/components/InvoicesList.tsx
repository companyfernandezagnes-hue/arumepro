...
import { Factura, AppData } from '../types';
  ...
  const historyList = useMemo(() => {
    try {
      return facturas.filter(f => {
        ...
        if (mode === 'proveedor') {
          const isSocio = sociosReales.some(rp => superNorm(f.cliente).includes(superNorm(rp)));
          if (isSocio) return false;
          if (f.cliente && superNorm(f.cliente) !== 'arume' && superNorm(f.cliente) !== 'zdiario') return false;
        } else {
          const isSocio = sociosReales.some(rp => superNorm(f.cliente).includes(superNorm(rp)) || superNorm(f.prov).includes(superNorm(rp)));
          if (!isSocio) return false;
        }
