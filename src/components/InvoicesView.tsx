...
export const AlbaranesView = ({ data, onSave }: AlbaranesViewProps) => {
  ...
  // 🛡️ SALVAVIDAS: Si el localStorage no tiene socios, usamos estos por defecto
  ...
  // 🛡️ GUARDADO BLINDADO CONTRA FALLOS SILENCIOSOS
  const handleSaveEdits = async (e?: React.MouseEvent) => {
    if (e) e.preventDefault(); // Impide que recargue la página web
    
    try {
      if (!editForm) return;
      
      // 1. Copia segura selectiva (No clonamos TODO data porque puede reventar si hay datos circulares)
      const newData = { 
        ...safeData, 
        albaranes: safeData.albaranes ? [...safeData.albaranes] : [] 
      };
      ...
      // 2. Limpieza de datos (evita NaNs)
      ...
      // 4. Inyectar datos modificados
      newData.albaranes[index] = {
        ...newData.albaranes[index], // Mantiene datos originales ocultos (como las fotos base64)
        ...editForm,
      ...
      // 5. Guardar
      await onSave(newData);
      setEditForm(null); // Cierra el modal
      alert("✅ Albarán actualizado y guardado correctamente.");

    } catch (error) {
      console.error(error);
      alert("⚠️ Hubo un error al guardar. Revisa que no hayas dejado celdas de precios vacías.");
    }
  };
