...
export const AlbaranesView = ({ data, onSave }: AlbaranesViewProps) => {
  ...
  const processImageWithAI = async (file: File | Blob) => {
    ...
    try {
      ...
      rec.lineas.forEach(nl => {
        ...
        if (lastPrice && currentUnit > lastPrice * 1.05) alerts.push({ n: nl.name, old: lastPrice, new: currentUnit });
      });
  ...
  useEffect(() => {
    const handlePaste = (e: ClipboardEvent) => {
      if (isAnalyzing || recordingMode || pasteLock.current) return;
      const items = e.clipboardData?.items;
      if (!items) return;
      for (let i = 0; i < items.length; i++) {
        if (items[i].type.indexOf("image") !== -1) {
          const blob = items[i].getAsFile();
          if (blob) { pasteLock.current = true; processImageWithAI(blob).finally(() => pasteLock.current = false); break; }
        }
      }
    };
  ...
  const processVoice = async (audioBlob: Blob, mimeType: string, mode: 'new' | 'edit') => {
    ...
    try {
      ...
      const cleanM = mimeType.split(';')[0];
      let prompt = mode === 'new' ? PROMPT_VOICE_NEW : `Aquí tienes el JSON de un albarán actual:
  ...
  const handleSaveAlbaran = async () => {
    ...
    const existingIdx = newData.albaranes.findIndex(a => 
      !a.invoiced && norm(a.prov) === norm(form.prov) && a.date === form.date && a.socio === form.socio && a.unitId === form.unitId
    );

    if (existingIdx !== -1 && !form.forceDup) {
      const existing = newData.albaranes[existingIdx];
      const newItems = analyzedItems.filter(newItem => !(existing.items || []).some((oldItem: any) => norm(oldItem.n) === norm(newItem?.n || '') && Math.abs((oldItem.t || 0) - (newItem?.t || 0)) < 0.01));
    ...
    setPriceAlerts([]); if (scannedImage) URL.revokeObjectURL(scannedImage); setScannedImage(null); 
  ...
  const handleSaveEdits = async () => {
    ...
    if (index !== -1) {
      // Reconciliamos antes de guardar para cuadrar IVAs matemáticamente
      const al: AlbaranIA = {
  ...
  const kpis = useMemo(() => {
    ...
    (data.albaranes || []).forEach(a => {
      ...
      const val = Num.parse(a.total); totalGlobal += val; const d = new Date(a.date);
    ...
    <div className={cn("animate-fade-in space-y-6 pb-24", isAnalyzing && "transition-none")}>
      ...
      <header className="flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100 gap-4">
        ...
        <div className="flex gap-2 items-center flex-wrap justify-center">
          ...
          <button 
            onClick={async () => {
              ...
              (newData.albaranes || []).forEach(a => {
                const targetKey = Object.keys(grouped).find(k => !grouped[k].invoiced && norm(grouped[k].prov) === norm(a.prov) && grouped[k].date === a.date && (grouped[k].unitId || 'REST') === (a.unitId || 'REST'));
              ...
              await onSave(newData); alert("¡Albaranes agrupados con éxito!");
            }}
          ...
          <button onClick={() => startVoiceRecording('new')} disabled={isAnalyzing} className={cn("px-4 py-3 rounded-2xl text-[10px] font-black uppercase transition-all shadow-md flex items-center justify-center gap-2", recordingMode === 'new' ? "bg-rose-500 text-white" : "bg-slate-900 text-white hover:bg-slate-800 disabled:opacity-50")}>
          ...
          <label className={cn("bg-gradient-to-r from-indigo-500 to-purple-600 text-white px-5 py-3 rounded-2xl text-[10px] font-black transition shadow-md flex items-center gap-2", isAnalyzing ? "opacity-50 cursor-wait" : "hover:shadow-lg hover:scale-105 cursor-pointer")}>
            <Camera className="w-4 h-4" /> <span>SUBIR (Ctrl+V)</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white px-6 py-5 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col justify-center items-start">
          <span className="text-[10px] font-black text-slate-400 uppercase mb-1">Gasto Histórico</span><span className="text-2xl font-black text-slate-800">{Num.fmt(kpis.totalGlobal)}</span>
        </div>
        <div className="bg-indigo-50 px-6 py-5 rounded-[2rem] border border-indigo-100 shadow-sm flex flex-col justify-center items-start relative overflow-hidden">
          <Clock className="absolute -right-4 -top-4 w-24 h-24 opacity-10 text-indigo-500" /><span className="text-[10px] font-black text-indigo-500 uppercase mb-1">Este Trimestre</span><span className="text-3xl font-black text-indigo-900">{Num.fmt(kpis.totalTrim)}</span>
        </div>
        <div className="bg-emerald-50 px-6 py-5 rounded-[2rem] border border-emerald-100 shadow-sm flex flex-col justify-center items-start relative overflow-hidden">
          <CheckCircle2 className="absolute -right-4 -top-4 w-24 h-24 opacity-10 text-emerald-500" /><span className="text-[10px] font-black text-emerald-600 uppercase mb-1">Este Mes</span><span className="text-3xl font-black text-emerald-900">{Num.fmt(kpis.totalMes)}</span>
        </div>
      </div>
          ...
          <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border-2 border-indigo-50 relative overflow-hidden">
            ...
            <div className="mt-3 space-y-1 max-h-52 overflow-y-auto custom-scrollbar px-1 bg-slate-50/50 rounded-xl p-2 min-h-[50px]">
              {analyzedItems.length > 0 ? analyzedItems.map((it, idx) => it && (
                <div key={idx} className="flex justify-between items-center text-[10px] border-b border-slate-200 py-2 last:border-0"><span className="truncate pr-2 font-bold text-slate-700"><b>{it.q}x</b> {it.n} <span className="text-[8px] text-slate-400">({it.rate}%)</span></span><span className="font-black text-slate-900 whitespace-nowrap">{Num.fmt(it.t)}</span></div>
            ...
            <div className="mt-4 p-4 bg-slate-900 rounded-2xl shadow-lg space-y-2">
              <div className="flex justify-between items-center pt-2 border-t border-slate-700 mt-2"><span className="text-xs font-black text-white uppercase">TOTAL</span><span className="text-2xl font-black text-white">{Num.fmt(liveTotals.grandTotal)}</span></div>
            </div>

            <button disabled={isAnalyzing} onClick={handleSaveAlbaran} className="w-full mt-2 bg-indigo-600 text-white py-4 rounded-2xl font-black shadow-xl hover:bg-indigo-700 transition active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed">GUARDAR COMPRA</button>
          </div>
        </div>
          ...
          <div className="space-y-3 pb-20">
            {filteredAlbaranes.length > 0 ? filteredAlbaranes.map(a => {
              ...
              <div className="py-20 text-center opacity-50 bg-slate-50 rounded-[3rem] border-2 border-dashed border-slate-200">
                <Truck className="w-12 h-12 mx-auto mb-3 text-slate-300" /><p className="text-slate-500 font-bold text-sm">Sin registros.</p>
              </div>
            )}
          </div>
        </div>
      ...
      <AnimatePresence>
        {editingAlbaran && editForm && (
          <div className="fixed inset-0 z-[200] flex justify-center items-center p-4">
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} className="bg-white w-full max-w-2xl rounded-[2.5rem] p-6 md:p-8 shadow-2xl relative z-10 flex flex-col max-h-[90vh]">
