...
export const DashboardView = ({ data }: { data: AppData }) => {
  ...
  const calculateStatsForPeriod = useCallback((month: number, quarter: number, year: number, mode: string) => {
    ...
    cierres.forEach((c: any) => {
      if (isDateInPeriod(c?.date)) {
        ...
        const t = Num.parse(c?.totalVenta || 0) || 0;
    ...
    facturas.forEach((f: any) => {
      if (isDateInPeriod(f?.date) && f?.status !== 'draft') {
        const tRaw = Num.parse(f?.total || 0) || 0;
  ...
  const renderTrend = (current: number, previous: number) => {
    ...
    if (!Number.isFinite(pct) || Number.isNaN(pct)) return null;
  ...
  const projection = useMemo(() => {
    ...
    return { inc: projectedIncome || 0, exp: projectedExpenses || 0, net: (projectedIncome - projectedExpenses) || 0 };
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-indigo-600 p-6 rounded-[2rem] shadow-md text-white flex flex-col justify-center relative overflow-hidden">
          ...
          <p className="text-[10px] font-bold text-indigo-200 mt-2 uppercase tracking-widest">
            F.C: {safeFixed(stats.ratios.foodCost)}% | L.C: {safeFixed(stats.ratios.laborCost)}%
          </p>
        </div>
      </div>

      {/* 🔮 PANEL PREDICTIVO DE IA */}
      {projection && (
        ...
        <div className="w-full h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData || []}>
              ...
              <YAxis axisLine={false} tickLine={false} tick={{fill: '#94a3b8', fontSize: 11, fontWeight: 800}} tickFormatter={(v) => Num.fmt(v || 0)} width={60} />
              <Tooltip 
                contentStyle={{borderRadius: '20px', border: 'none', boxShadow: '0 10px 25px -5px rgb(0 0 0 / 0.1)', fontWeight: 800, padding: '12px'}}
                formatter={(value: number) => [Num.fmt(value || 0), 'Ventas']}
              />
        ...
        <div className="bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col h-full">
          ...
          <div className="space-y-6 flex-1">
            {[
              ...
              <div key={g.label || i} className="space-y-2 group">
                <div className="flex justify-between text-[11px] font-black uppercase tracking-widest">
                  <span className="text-slate-500 flex items-center gap-2">
                    <g.icon className="w-3.5 h-3.5" style={{ color: g.hex }} /> {g.label}
                  </span>
                  <span className="text-slate-900 group-hover:scale-110 transition-transform">{Num.fmt(g.val || 0)}</span>
                </div>
        ...
        <div className="bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-100 shadow-xl relative overflow-hidden h-full">
          ...
          <div className="space-y-6">
            {BUSINESS_UNITS.map(unit => {
              ...
              const maxIncome = Math.max(...Object.values(unitBreakdown).map(u => u.income || 0), 1);
              return (
                <div key={unit.id} className="group">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <div className="text-right">
                      <span className={cn("text-base font-black tracking-tighter", (uStat.profit || 0) >= 0 ? "text-emerald-600" : "text-rose-600")}>
                        {Num.fmt(uStat.profit || 0)}
                      </span>
                    </div>
                  </div>
                  ...
                  <p className="text-[9px] font-bold text-slate-400 mt-1.5 flex justify-between uppercase tracking-widest px-1">
                    ...
                    <span className="text-rose-400">Gastos: {Num.fmt(uStat.expenses || 0)}</span>
                  </p>
                </div>
              );
            })}
        ...
        <div className="flex flex-col gap-6 h-full">
          ...
          <div className="bg-slate-900 p-6 rounded-[2.5rem] border border-slate-800 shadow-xl flex-1 flex flex-col relative overflow-hidden group min-h-[220px]">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2 relative z-10 pr-2">
              {loadingEmails ? (
                ...
                generalEmails.map((email, index) => (
                  <div key={email?.id || index} className="bg-slate-800/80 p-3 rounded-2xl border border-slate-700 hover:border-blue-500/50 hover:bg-slate-800 transition">
                    <div className="flex justify-between items-start">
                      <p className="text-xs font-bold text-white truncate max-w-[70%]">{email?.remitente || 'Desconocido'}</p>
                    </div>
                    <p className="text-[10px] text-slate-400 truncate mt-1 leading-tight">{email?.asunto || 'Sin asunto'}</p>
                  </div>
                ))
              )}
            </div>
          ...
          <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm flex-1 min-h-[200px]">
            ...
            <div className="space-y-3 overflow-y-auto custom-scrollbar pr-2">
              ...
              {lowStock.map((ing, i) => (
                <div key={ing?.id || i} className="flex items-center gap-3 p-3 bg-rose-50 rounded-2xl border border-rose-100">
                  <AlertCircle className="w-5 h-5 text-rose-500 shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-bold text-rose-900 truncate">{ing?.n || 'Desconocido'}</p>
                    <p className="text-[9px] text-rose-500 font-black uppercase tracking-widest">Stock: {ing?.stock || 0}</p>
                  </div>
                </div>
              ))}
