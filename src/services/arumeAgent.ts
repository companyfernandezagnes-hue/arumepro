...
export class ArumeAgent {
  ...
  // ── Telegram DESACTIVADO — no enviar mensajes automáticos ──
  private static async _sendTelegram(_data: AppData, _text: string): Promise<boolean> {
    // Cortado: la app ya no envía mensajes a Telegram automáticamente.
    // Los datos de config (token/chatId) se conservan por si se reactiva.
    return false;
  }
